...
class PointProgressCircleView2 @JvmOverloads constructor(context: Context?, attrs: AttributeSet?=null, defStyleAttr: Int=0) : ViewGroup(context, attrs, defStyleAttr) {
    ...
    override fun onLayout(changed: Boolean, l: Int, t: Int, r: Int, b: Int) {
        if(changed){
            //计算每个点的中心位置
            list.clear()
            val radius = (measuredWidth - pointRB * 2) / 2
            for (i in 0 until count) {
                list.add(
                        Triple(radius * cos(360 / count * i)+radius+pointRB,
                                radius * sin(360 / count * i)+radius+pointRB,
                                caculateColor(startColor,endColor,(i+1)/count.toFloat()))
                )
            }
            //布局每个点
    ...
    fun setProgress(percent: Float){
        ...
        filter.forEachIndexed { index, _ ->
            if(index==count/2 && interval){
                //不会缩小
                points[index]?.start(0)
            }else if(index==filter.size-1){
                points[index]?.start(0)
            }else{
